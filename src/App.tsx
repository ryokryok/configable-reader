...
function App() {
	...
	const textStyle = {
		fontSize: `${state.font_size}em`,
		fontFamily: state.font_family,
		lineHeight: `${state.line_height}em`
	};
	return (
			...
			<DispatchContext.Provider value={dispatch}>
				<div className="App">
					...
					<div style={textStyle}>
						...
						作者はさっき、「下人が雨やみを待っていた」と書いた。しかし、下人は雨がやんでも、格別どうしようと云う当てはない。ふだんなら、勿論、主人の家へ帰る可き筈である。所がその主人からは、四五日前に暇を出された。前にも書いたように、当時京都の町は一通りならず衰微すいびしていた。今この下人が、永年、使われていた主人から、暇を出されたのも、実はこの衰微の小さな余波にほかならない。だから「下人が雨やみを待っていた」と云うよりも「雨にふりこめられた下人が、行き所がなくて、途方にくれていた」と云う方が、適当である。その上、今日の空模様も少からず、この平安朝の下人の
						Sentimentalisme
						に影響した。申さるの刻こく下さがりからふり出した雨は、いまだに上るけしきがない。そこで、下人は、何をおいても差当り明日あすの暮しをどうにかしようとして――云わばどうにもならない事を、どうにかしようとして、とりとめもない考えをたどりながら、さっきから朱雀大路にふる雨の音を、聞くともなく聞いていたのである。
						...
						老婆は、一目下人を見ると、まるで弩いしゆみにでも弾はじかれたように、飛び上った。 「おのれ、どこへ行く。」
...
type ActionType = ConfigState & {
	type: 'reset' | 'update_font_size' | 'update_font_family' | 'update_line_height' | 'update_mode';
};

function reducer(state: ConfigState, action: ActionType) {
	switch (action.type) {
		case 'reset':
			return initialState;
...
const ConfigContext = createContext<ConfigState>(null as any);
const DispatchContext = createContext<Dispatch<ActionType>>(null as any);
